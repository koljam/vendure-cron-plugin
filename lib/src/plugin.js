...
let CronPlugin = CronPlugin_1 = class CronPlugin {
    /** @internal */
    constructor(eventBus, requestContextService) {
    ...
    async onApplicationBootstrap() {
        CronPlugin_1.options.cron.forEach((job) => {
            if (!job.task && !job.taskId) {
                common_1.Logger.error('Please provide either a task function or a taskId to run a cronjob.', constants_1.loggerCtx);
                throw new Error('LOL');
                return;
            }
...
CronPlugin = CronPlugin_1 = __decorate([
    ...
    __metadata("design:paramtypes", [core_1.EventBus, core_1.RequestContextService])
], CronPlugin);
